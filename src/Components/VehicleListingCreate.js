...
class VehicleListingCreate extends Component {
    ...
    componentDidMount() {
        if (this.state.years.length < 1) {
            fetch(this.props.BACKEDN + '/api/v1/auto_info/get_years')
                .then(res => {
                    return res.json()
                }).then(data => {
                    this.setState({
    ...
    componentDidUpdate() {
        if (this.state.selYear) {
            if (this.state.makes.length < 1) {
                fetch(this.props.BACKEDN + '/api/v1/auto_info/get_makes?year=' + this.state.selYear)
                    .then(res => {
                        return res.json()
                    }).then(data => {
                        this.setState({
                ...
                fetch(this.props.BACKEDN + '/api/v1/auto_info/get_models?year=' + this.state.selYear + '&make=' + this.state.selMake)
                    .then(res => {
                        return res.json()
                    }).then(data => {
                        this.setState({
                ...
                fetch(this.props.BACKEDN + '/api/v1/auto_info/get_options?year=' + this.state.selYear + '&make=' + this.state.selMake + '&model=' + this.state.selModel)
                    .then(res => {
                        return res.json()
                    }).then(data => {
                        this.setState({
    ...
    formSubmition = (event) => {
        ...
        console.log(this.state)
        fetch(this.props.BACKEDN + '/api/v1/listings/create_listing', {
            method: 'POST',
            credentials: 'include',
            headers: {
